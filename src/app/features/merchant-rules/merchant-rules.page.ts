...
@Component({
  ...
  `,
    ...
    .wrap { padding: 12px 14px; max-width: 480px; margin: 0 auto; }

    .intro-card { display: flex; gap: 12px; align-items: flex-start; background: white; border-radius: 16px; padding: 14px; margin-bottom: 14px; border: 1px solid #e8eaed; }
    .intro-icon { font-size: 28px; flex-shrink: 0; }
    .intro-title { font-family: 'Syne', sans-serif; font-size: 14px; font-weight: 800; color: #1e293b; margin-bottom: 3px; }
    .intro-sub { font-size: 12px; color: #6b7280; line-height: 1.5; }

    .search-wrap { position: relative; margin-bottom: 12px; }
    .search-icon { position: absolute; left: 13px; top: 50%; transform: translateY(-50%); font-size: 16px; color: #9ca3af; }
    .search-input { width: 100%; padding: 10px 14px 10px 38px; border: 1.5px solid #e2e8f0; border-radius: 12px; background: white; font-family: 'DM Sans', sans-serif; font-size: 14px; outline: none; }

    .stats-row { display: flex; gap: 8px; margin-bottom: 12px; }
    .stat-pill { flex: 1; background: white; border: 1px solid #e8eaed; border-radius: 12px; padding: 10px; text-align: center; }
    .stat-pill.green { background: #ecfdf5; border-color: #a7f3d0; }
    .stat-pill.gold  { background: #fffbeb; border-color: #fde68a; }
    .stat-num { display: block; font-family: 'Syne', sans-serif; font-size: 20px; font-weight: 800; color: #1e293b; }
    .stat-lbl { font-size: 10px; color: #9ca3af; text-transform: uppercase; letter-spacing: 0.6px; }

    .cat-tabs { display: flex; gap: 6px; overflow-x: auto; padding-bottom: 4px; margin-bottom: 12px; -ms-overflow-style: none; scrollbar-width: none; }
    .cat-tabs::-webkit-scrollbar { display: none; }
    .cat-tab { flex-shrink: 0; padding: 6px 12px; border-radius: 20px; border: 1.5px solid #e2e8f0; background: white; font-size: 13px; cursor: pointer; transition: all 0.15s; }
    .cat-tab.active { background: var(--tab-color, #6366f1); border-color: var(--tab-color, #6366f1); color: white; }

    .loading-wrap { text-align: center; padding: 48px; }
    .empty-wrap { text-align: center; padding: 48px 24px; color: #6b7280; }

    .rules-list { display: flex; flex-direction: column; gap: 10px; }
    .rule-card { background: white; border-radius: 16px; padding: 14px; border: 1px solid #e8eaed; transition: opacity 0.2s; }
    .rule-card.disabled { opacity: 0.45; }

    .rule-top { display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px; }
    .rule-pattern { display: flex; align-items: center; gap: 7px; flex: 1; min-width: 0; }
    .match-badge { font-size: 9px; font-weight: 800; padding: 2px 7px; border-radius: 20px; text-transform: uppercase; letter-spacing: 0.5px; flex-shrink: 0; }
    .match-contains   { background: #eff6ff; color: #1e40af; }
    .match-startsWith { background: #f0fdf4; color: #166534; }
    .match-exact      { background: #fdf4ff; color: #86198f; }
    .match-regex      { background: #fff7ed; color: #9a3412; }
    .pattern-text { font-family: 'JetBrains Mono', 'Courier New', monospace; font-size: 13px; font-weight: 600; color: #1e293b; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .rule-toggle { --handle-width: 18px; --handle-height: 18px; width: 42px; height: 24px; flex-shrink: 0; }

    .rule-arrow { display: flex; align-items: center; gap: 8px; margin-bottom: 8px; }
    .arrow-line { color: #9ca3af; font-size: 16px; }
    .rule-result { display: flex; align-items: center; gap: 6px; flex: 1; }
    .cat-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
    .cat-label { font-size: 13px; font-weight: 600; color: #374151; }
    .subcat-label { font-size: 12px; color: #6b7280; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .hit-badge { background: #f3f4f6; border-radius: 20px; padding: 2px 8px; font-size: 11px; font-weight: 700; color: #374151; flex-shrink: 0; }

    .rule-actions { display: flex; align-items: center; gap: 8px; }
    .priority-chip { font-size: 10px; font-weight: 700; background: #f3f4f6; border-radius: 6px; padding: 2px 7px; color: #6b7280; }
    .action-btn { display: inline-flex; align-items: center; gap: 4px; padding: 5px 10px; border-radius: 8px; border: 1px solid #e2e8f0; background: #f8fafc; font-size: 12px; font-weight: 600; cursor: pointer; color: #374151; }
    .action-btn.danger { color: #dc2626; background: #fef2f2; border-color: #fecaca; }
    .action-btn ion-icon { font-size: 13px; }

    /* Sheet */
    .sheet-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0); z-index: 500; pointer-events: none; transition: background 0.3s; display: flex; align-items: flex-end; }
    .sheet-overlay.open { background: rgba(0,0,0,0.5); pointer-events: all; }
    .sheet { width: 100%; max-width: 480px; margin: 0 auto; background: white; border-radius: 24px 24px 0 0; padding: 10px 18px 32px; max-height: 90vh; overflow-y: auto; transform: translateY(100%); transition: transform 0.35s cubic-bezier(0.16,1,0.3,1); }
    .sheet-overlay.open .sheet { transform: translateY(0); }
    .sheet-handle { width: 36px; height: 4px; background: #e2e8f0; border-radius: 2px; margin: 0 auto 16px; }
    .sheet-title { font-family: 'Syne', sans-serif; font-size: 18px; font-weight: 800; margin-bottom: 18px; color: #0f172a; }

    .field-group { margin-bottom: 14px; }
    .field-row { display: flex; gap: 12px; align-items: flex-start; }
    .field-label { display: block; font-size: 10px; font-weight: 700; color: #9ca3af; text-transform: uppercase; letter-spacing: 0.8px; margin-bottom: 7px; }
    .field-hint { font-size: 11px; color: #9ca3af; margin-top: 5px; line-height: 1.5; }
    .sheet-input { width: 100%; padding: 11px 13px; border: 1.5px solid #e2e8f0; border-radius: 12px; font-family: 'DM Sans', sans-serif; font-size: 14px; outline: none; transition: border-color 0.15s; }
    .sheet-input:focus { border-color: #6366f1; }

    .match-type-row { display: flex; gap: 6px; flex-wrap: wrap; }
    .match-type-btn { padding: 6px 12px; border: 1.5px solid #e2e8f0; border-radius: 20px; background: white; font-size: 12px; font-weight: 600; cursor: pointer; color: #6b7280; transition: all 0.15s; }
    .match-type-btn.active { background: #0f172a; border-color: #0f172a; color: white; }

    .cat-chip-row { display: flex; flex-wrap: wrap; gap: 6px; }
    .cat-chip { padding: 6px 12px; border: 1.5px solid #e2e8f0; border-radius: 20px; background: white; font-size: 12px; font-weight: 600; cursor: pointer; color: #374151; transition: all 0.15s; }
    .cat-chip.active { background: var(--chip-color); border-color: var(--chip-color); color: white; }

    .subcat-chip-row { display: flex; flex-wrap: wrap; gap: 6px; }
    .subcat-chip { padding: 5px 11px; border: 1.5px solid #e2e8f0; border-radius: 20px; background: white; font-size: 12px; cursor: pointer; color: #374151; transition: all 0.15s; }
    .subcat-chip.active { background: #6366f1; border-color: #6366f1; color: white; }

    .test-section { background: #f8fafc; border-radius: 12px; padding: 12px; margin-bottom: 16px; }
    .test-label { font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.6px; margin-bottom: 8px; }
    .test-result { font-size: 12px; font-weight: 700; margin-top: 8px; padding: 6px 10px; border-radius: 8px; }
    .test-result.match { background: #dcfce7; color: #166534; }
    .test-result.no-match { background: #fef2f2; color: #991b1b; }

    .save-btn { width: 100%; padding: 14px; border-radius: 14px; border: none; background: #6366f1; color: white; font-family: 'Syne', sans-serif; font-size: 15px; font-weight: 800; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; }
